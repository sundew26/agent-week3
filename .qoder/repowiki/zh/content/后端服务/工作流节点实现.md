...
## 更新摘要
**变更内容**
- 移除了审核节点作为独立步骤，工作流程简化为 plan→search→analyze→write→output/revision
- 保留了审核相关状态字段和API接口，支持条件分支逻辑
- 更新了工作流图结构，从七节点简化为六节点流程
- 优化了条件路由逻辑，通过迭代计数控制修订循环
...
## 简介
本技术文档围绕"AI 研究工作流"的六个核心节点进行深度解析，包括：规划节点（问题拆解）、搜索节点（网络搜索）、分析节点（结果分析）、写作节点（报告生成）、修订节点（反馈处理）、输出节点（最终报告）。工作流程已简化为线性流程加条件分支模式，移除了独立的审核节点，通过迭代计数机制实现自动化的质量控制和循环修订保护。文档覆盖每个节点的输入输出、LLM 调用逻辑、工具使用方式、错误处理策略；并说明节点间数据传递机制、上下文共享方式、性能优化技巧。最后提供节点开发模板与扩展指南，帮助开发者快速添加新的工作流节点。
...
- 节点集合：plan/search/analyze/write/revision/output 六个节点（移除了独立的 review 节点）
...
## 架构总览
下图展示了从用户提问到最终报告的完整执行路径，包含条件分支与循环修订保护。工作流程已简化为线性流程，在写作节点后直接根据迭代计数决定输出或修订。
...
G->>N : route_after_review(state)
alt iteration_count < 3
...
- 路由逻辑：当 iteration_count < 3 时，回到 search 重新搜索；否则强制输出

**更新** 修订节点现在直接由条件路由函数控制，不再需要独立的审核节点
...
**更新** 输出节点现在由条件路由函数直接控制，不再经过独立的审核节点
...
- 工作流中断：确认 interrupt_before 配置正确，检查 graph.aget_state 是否返回 next 为空
...
## 结论
该工作流以 LangGraph 为核心，结合 FastAPI 与 SSE 实现了端到端的 AI 研究助手可视化流程。工作流程已简化为六个核心节点，移除了独立的审核节点，通过条件路由和迭代计数机制实现自动化的质量控制。整体架构清晰、可扩展性强，适合在生产环境中替换工具与增强错误处理。简化的流程设计提高了执行效率，同时保持了必要的质量控制能力。
